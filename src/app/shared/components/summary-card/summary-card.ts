...
@Component({
  ...
  styles: ['article{height:100%;padding:var(--space-4);background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-lg);box-shadow:var(--shadow-sm)}.heading{display:flex;align-items:flex-start;justify-content:space-between;gap:var(--space-2)}h2{margin:0;color:var(--color-text-muted);font-size:var(--font-size-sm)}.value{margin:var(--space-3) 0 0;font-size:var(--font-size-2xl);font-weight:800;letter-spacing:-.02em}.context{margin:var(--space-2) 0 0;color:var(--color-text-muted);font-size:var(--font-size-xs)}@media(max-width:64rem){.heading{flex-direction:column}.value{font-size:clamp(1.25rem,5vw,1.5rem)}}'],
