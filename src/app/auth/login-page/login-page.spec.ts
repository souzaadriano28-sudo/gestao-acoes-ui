...
describe('LoginPageComponent',()=>{
  ...
  it('prioriza acesso e mantém segurança e finalidade acadêmica sob texto conciso',()=>{const fixture=TestBed.createComponent(LoginPageComponent);fixture.detectChanges();http.expectOne('/api/auth/csrf').flush({token:'x',headerName:'X-CSRF-TOKEN',parameterName:'_csrf'});fixture.detectChanges();const root=fixture.nativeElement as HTMLElement;expect(root.querySelector('#acesso form')).toBeTruthy();expect(root.querySelector('.security-details')?.textContent).toContain('cookie HttpOnly');expect(root.querySelector('.notice')?.textContent).toContain('Finalidade acadêmica');expect(root.querySelectorAll('.signal')).toHaveLength(0);});
  it('aceita identificador de login com o mesmo limite de e-mail da API',()=>{
    const fixture=TestBed.createComponent(LoginPageComponent);fixture.detectChanges();http.expectOne('/api/auth/csrf').flush({token:'x',headerName:'X-CSRF-TOKEN',parameterName:'_csrf'});fixture.detectChanges();
    const root=fixture.nativeElement as HTMLElement;expect(root.querySelector('label[for=usuario]')?.textContent).toContain('e-mail');
    const control=root.querySelector('#usuario') as HTMLInputElement;control.value=`${'a'.repeat(240)}@example.test`;control.dispatchEvent(new Event('input'));
    expect(fixture.componentInstance.form.controls.username.valid).toBe(true);
  });
});
