...
describe('authInterceptor',()=>{
  ...
  it('aceita apenas retornos internos conhecidos',()=>{expect(safeReturnUrl('/acoes?x=1')).toBe('/acoes?x=1');expect(safeReturnUrl('https://evil.invalid')).toBe('/carteira');expect(safeReturnUrl('//evil.invalid')).toBe('/carteira');expect(safeReturnUrl('/login')).toBe('/carteira');});
});
